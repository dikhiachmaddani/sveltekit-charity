<script lang="ts">
	...
	function calculatedFounded(pledged: number, target: number): number {
		return Math.round((1 / (target / pledged)) * 100);
	}
	function formatCurrency(nominal: number): string {
		return nominal.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' });
	}
	function dateDayRemaining(date_end: Date): number {
		const delta = date_end.getTime() - new Date().getTime();
	...
	<div class="container">
		<div class="xs-heading row xs-mb-60">
			<div class="col-md-9 col-xl-9">
				...
				<p>
					FundPress has built a platform focused on aiding entrepreneurs, startups, and <br /> companies
						...
						<div class="xs-popular-item xs-box-shadow">
									...
									<div class="xs-skill-track">
										<p>
											<span
												...
												>{calculatedFounded(charity.pledged, charity.target)}</span
											>%
										</p>
									</div>
								</div>
							...
							<div class="xs-item-content">
								...
								<ul class="xs-list-with-content">
									...
									<li>{dateDayRemaining(charity.date_end)}<span>Days to go</span></li>
								</ul>
